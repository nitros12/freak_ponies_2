...
static const String:ff2versiondates[][]=
{
	...
	"November 6, 2014",	//1.10.3
	"November 6, 2014",	//1.10.3
	"January 1, 2015" //nitros'/ night's edit
};
...
stock FindVersionData(Handle:panel, versionIndex)
{
	switch(versionIndex)
	{
		case 54: //"Nitros'/ Night's edit",
		{
			DrawPanelText(panel, "1) Fixed some minor bugs");
			DrawPanelText(panel, "2) Demoshields no longer provide a 'life'");
			DrawPanelText(panel, "3) Backstabs and market gardens have been nerfed");
			DrawPanelText(panel, "4) Different mediguns and syringe guns can now be used (not including vaccinator)");
			DrawPanelText(panel, "5) Lots of weapon balances to make the game more interesting");
...
{
	name="Freak Ponies 2",
	author="Rainbolt Dash, FlaminSarge, Powerlord, the 50DKP team, Night",
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	//Scout
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "278 ; 0.6 ; 69 ; 0.70");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Soldier
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "178 ; 0.75 ; 3 ; 0.66 ; 265 ; 5", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 444:  //Mantreads
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "70 ; 1.50 ; 239 ; 1.15 ; 77 ; 0.8");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Pyro
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "25 ; 0.5 ; 207 ; 1.33 ; 58 ; 3.2 ; 71 ; 2.0 ; 144 ; 1", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Demoman
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "412 ; 0.6 ; 77 ; 0.75");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 406, 1099:  //Splendid Screen, Tide Turner
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "412 ; 0.85");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Heavy
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "77 ; 0.0 ; 2 ; 1.3 ; 4 ; 1.34 ; 106 ; 0.8 ; 29 ; 1 ; 45 ; 1.2 ; 69 ; 0.6 ; 16 ; 10", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Medic
		...
		case 36:  //Blutsauger
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "17 ; 0.06 ; 2 ; 1.1 ; 3 ; 0.50 ; 15 ; 1");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 412:  //Overdose
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "17 ; 0.02 ; 97 ; 0.8 ; 1 ; 0.9 ; 6 ; 0.7 ; 3 ; 0.75 ; 77 ; 0.8", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 17, 204:  //Syringe gun
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "17 ; 0.05", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		//Secondary
		case 29, 35, 211, 663, 796, 805, 885, 894, 903, 912, 961, 970:  //Renamed/Strange, Festive, Silver Botkiller, Gold Botkiller, Rusty Botkiller, Bloody Botkiller, Carbonado Botkiller, Diamond Botkiller Mk.II, Silver Botkiller Mk.II, and Gold Botkiller Mk.II Mediguns, Kritzkrieg
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "10 ; 1.25 ; 178 ; 0.75 ; 144 ; 2.0 ; 11 ; 1.15");
				//10: +25% faster charge rate
				//178: +25% faster weapon switch
				//144: Quick-fix speed/jump effects
				//11: +15% overheal bonus
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 411:  //Quick-fix
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "18 ; 1 ; 8 ; 2.0 ; 10 ; 1.4 ; 231 ; 1 ; 11 ; 1.1");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	//Sniper
		...
		case 1098:	//Classic
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "91 ; 0.6 ; 6 ; 0.7");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 57:  //Razorback
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "77 ; 0.8 ; 412 ; 0.6");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
...
public Action:MakeNotBoss(Handle:timer, any:userid)
{
	...
	if(GetClientHealth(client) > 360 && GetClientTeam(client) != BossTeam) // A temporary workaround to this overheal bug
    {
        ForcePlayerSuicide(client);
        TF2_RespawnPlayer(client);
        CPrintToChat(client, "{olive}[FF2]{default} Respawning you because of an overheal bug!");
    } 
...
public Action:CheckItems(Handle:timer, any:client)  //Weapon balance 2
{
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			case 17, 36, 204, 412:  //Syringe Gun, Blutsauger, Strange Syringe Gun, Overdose
			{
				...
				{
					//TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary); removed being replaced, 
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			...
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
				weapon=SpawnWeapon(client, "tf_weapon_pistol_scout", 23, 1, 0, "");
			}
		}
	}
	else
	...
	if(TF2_GetPlayerClass(client)==TFClass_Medic)
	{
		...
		if(mediquality!=10)
		{
			...
			switch(index)
			{
				case 29, 35, 411, 211, 663, 796, 805, 885, 894, 903, 912, 961, 970:  //Renamed/Strange, Festive, Silver Botkiller, Gold Botkiller, Rusty Botkiller, Bloody Botkiller, Carbonado Botkiller, Diamond Botkiller Mk.II, Silver Botkiller Mk.II, and Gold Botkiller Mk.II Mediguns
				{
				...
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
					weapon=SpawnWeapon(client, "tf_weapon_medigun", 29, 5, 10, "10 ; 1.25 ; 178 ; 0.75 ; 144 ; 2.0 ; 11 ; 1.15");
...
public Action:ClientTimer(Handle:timer)
{
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			switch(index)
			{
				case 16, 56, 203, 305, 751, 1005, 1079, 1092, 1100:  //SMG, Huntsman, Strange SMG, Crusader's Crossbow, Cleaner's Carbine, Festive Huntsman, Festive Crossbow, Fortified Compound, Bread Bite
				{
					addthecrit=true;
				}
	...
	else
	{
		...
		if(boss!=-1)
		{
			if(attacker<=MaxClients)
			{
				...
				switch(index)
				{
					case 14, 201, 230, 402, 526, 664, 752, 792, 801, 851, 881, 890, 899, 908, 957, 966, 1098:  //Sniper Rifle, Strange Sniper Rifle, Sydney Sleeper, Bazaar Bargain, Machina, Festive Sniper Rifle, Hitman's Heatmaker, Botkiller Sniper Rifles, Classic
					{
						switch(index)
						{
					...
					case 416:  //Market Gardener (courtesy of Chdata)
					{
						...
						{
							damage=(Pow(float(BossHealthMax[boss]), (0.74074))+512.0-(Marketed[Boss[boss]]/128*float(BossHealthMax[boss])))/5.0; //changed from /3.0 to /5.0
							damagetype|=DMG_CRIT;
					...
					{
						SetEntPropFloat(attacker, Prop_Send, "m_flChargeMeter", 75.0);
					}
				...
				if(bIsBackstab)
				{
					damage=BossHealthMax[boss]*(LastBossIndex()+1)*BossLivesMax[boss]*(0.12-Stabbed[boss]/90)/4; //changed from /3 to /5
					damagetype|=DMG_CRIT;
					damagecustom=0;
